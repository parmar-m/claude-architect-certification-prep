...
|---|---|---|---|
| **What it is** | Claude Code's stateful, autonomous harness packaged as a library | A pre-built agent harness Anthropic hosts | **The primitive.** One request → one response |
...
> **Client SDKs** are a minimal stateless wrapper around Messages API (You have to manually manage conversation history, construct the loops, and handle function-calling, etc). Use if you don't need full harness, or if language is unsupported by Agent SDK.
> 
> Whereas **Agent SDK** is a stateful autonomous harness, (it manages the execution loop natively—running shell commands, reading/writing files, spawning sub-agents, and resolving tools, etc).
...
### (c) The two inbuilt harnesses - Tool Runner (Client SDK) & Agent SDK
...
> 1. Tool Runner's hooks are **post-execution only** - once Claude requests a tool, it runs instantly and the runner lets you inspect the result/error before it's sent back.
 However, there's no callback that fires *before* a tool executes. So if you need some handle before tool execution, such as **human-in-the-loop approval**, **custom logging** (no visibility into the raw request/response cycle), or **conditional execution** (no way to block/redirect a call before it runs), you must switch to the manual loop (or to Agent SDK).
>
...
> **The recurring-task trio is a common wrong answer set:**
